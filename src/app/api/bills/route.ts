...
export async function GET() {
  try {
    ...
    const billsQuery = `
      SELECT b.bill_id, c.name AS citizen, u.type AS utility, u.provider, 
             b.amount, b.due_date, b.payment_status,
             -- Aggregations per utility type
             COUNT(*) OVER (PARTITION BY u.type) AS bills_per_utility_type,
             AVG(b.amount) OVER (PARTITION BY u.type) AS avg_amount_per_type,
             SUM(b.amount) OVER (PARTITION BY u.type) AS total_revenue_per_type,
             -- Citizen spending analytics
             COUNT(*) OVER (PARTITION BY c.citizen_id) AS citizen_total_bills,
             SUM(b.amount) OVER (PARTITION BY c.citizen_id) AS citizen_total_amount,
             -- Payment status analytics
             COUNT(*) OVER (PARTITION BY b.payment_status) AS status_count,
             -- Overdue analysis
             CASE WHEN b.due_date < CURRENT_DATE AND b.payment_status = 'Unpaid' 
